...
class KynuxMongoBase extends EventEmitter {
  constructor(mongoConnectURL, connectionParams = {}) {
    ...
    if (!mongoConnectURL || !mongoConnectURL.startsWith('mongodb')) {
      throw new TypeError(
        'KynuxDB Mongo Error: A valid MongoDB connection URL must be provided.'
      );
    }
    if (typeof mongoConnectURL !== 'string') {
      throw new TypeError(
        `KynuxDB Mongo Error: Expected a string for mongoConnectURL, received ${typeof mongoConnectURL}`
      );
    }
    if (typeof connectionParams !== 'object' || connectionParams === null) {
      throw new TypeError(
        `KynuxDB Mongo Error: Expected an object for connectionParams, received ${typeof connectionParams}`
      );
    }
  ...
  _setupConnectionListeners() {
    ...
    connection.on('open', () => {
      console.log(
        '[KynuxDB Mongo] Successfully connected to MongoDB. Discord: https://discord.gg/wCK5dVSY2n'
      );
      this.emit('ready');
    });
  }
  ...
  _initializeConnection() {
    ...
    mongoose.connect(this.mongoURI, finalOptions).catch((err) => {
  ...
  _closeConnection() {
    mongoose
      ...
      .catch((err) =>
        console.error('KynuxDB Mongo Error: Failed to close connection.', err)
      );
  }
}
