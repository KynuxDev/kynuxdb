...
class YamlDB extends BaseAdapter {
  constructor(options) {
    ...
    try {
      ...
      throw new TypeError(
        "KynuxDB YAML Error: You must install the 'yaml' module to use this adapter. Run 'npm install yaml'."
      );
    }
  ...
  _readAllData() {
    ...
    try {
      ...
      try {
        ...
        console.error(
          `KynuxDB YAML Error: Failed to reset corrupted file ${this.filePath}:`,
          writeError
        );
      }
      return {};
  ...
  _writeAllData(yamlData) {
    try {
      ...
      this._cache = JSON.parse(JSON.stringify(yamlData));
      this._resetCacheTimeout();
    } catch (error) {
  ...
  _find(query, options = {}) {
    ...
    if (options.sort && typeof options.sort === 'object') {
      ...
      if (sortKeys.length > 0) {
        ...
        results.sort((a, b) => {
          const valA = getObjectValue(a, ...sortKey.split('.'));
          const valB = getObjectValue(b, ...sortKey.split('.'));
